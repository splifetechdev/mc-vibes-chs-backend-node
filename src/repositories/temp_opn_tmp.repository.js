...
exports.findTempOpnTmpAll = async (id) =>
  await db.sequelize.query(
    `SELECT * FROM temp_opn_tmp
     WHERE temp_opn_tmp.company_id = :id`,
    {
      replacements: { id },
...
exports.findTempOpnTmpAllByID = async (id) =>
  await db.sequelize.query(
    `SELECT * FROM temp_opn_tmp
     WHERE temp_opn_tmp.id = :id`,
    {
      replacements: { id },
      type: db.sequelize.QueryTypes.SELECT,
    }
...
exports.findDueDateByRouting = async (
  ...
  await db.sequelize.query(
    `SELECT opn_end_date_time FROM temp_opn_tmp
     WHERE  temp_opn_tmp.doc_running_no = :doc_running
     AND temp_opn_tmp.rtg_id = :rtg_id
     AND temp_opn_tmp.item_master_id = :item_master_id
     AND temp_opn_tmp.opn_id = :opn_id`,
    {
...
/*
  SELECT top 1 doc_running_no,opn_start_date_time,opn_end_date_time,overlab_time_cal,MAX(opn_end_date_time) FROM temp_opn_tmp
     WHERE  temp_opn_tmp.doc_running_no = N'PDCM-2400367'
     AND temp_opn_tmp.rtg_id = N'02'
     AND temp_opn_tmp.item_master_id = 10
     AND temp_opn_tmp.opn_id = N'100'
     GROUP BY doc_running_no,opn_start_date_time,opn_end_date_time,overlab_time_cal
  */

//  `SELECT *,MAX(opn_start_date_time) FROM temp_opn_tmp
//  WHERE  temp_opn_tmp.doc_running_no = :doc_running
//  AND temp_opn_tmp.rtg_id = :rtg_id
//  AND temp_opn_tmp.item_master_id = :item_master_id
//  AND temp_opn_tmp.opn_id = :opn_id`;
...
exports.findPOTempByOPNMINStartDate = async (
  ...
  await db.sequelize.query(
    `SELECT top 1 doc_running_no,opn_start_date_time,opn_end_date_time,overlab_time_cal,MAX(opn_start_date_time) FROM temp_opn_tmp
     WHERE  temp_opn_tmp.doc_running_no = :doc_running
     AND temp_opn_tmp.rtg_id = :rtg_id
     AND temp_opn_tmp.item_master_id = :item_master_id
     AND temp_opn_tmp.opn_id = :opn_id
...
// `SELECT *,MAX(opn_end_date_time) FROM temp_opn_tmp
//    WHERE  temp_opn_tmp.doc_running_no = :doc_running
//    AND temp_opn_tmp.rtg_id = :rtg_id
//    AND temp_opn_tmp.item_master_id = :item_master_id
//    AND temp_opn_tmp.opn_id = :opn_id`;
...
// `SELECT * FROM temp_opn_tmp tpopd
...
exports.findListByCompany = async (company_id) =>
  await db.sequelize.query(
    `SELECT 
        ...
        FROM temp_opn_tmp
     WHERE temp_opn_tmp.company_id = :company_id`,
    {
...
exports.create = async (data) => await db.temp_opn_tmp.create(data);

exports.update = async (doc_running, rtg_id, item_master_id, opn_id, data) =>
  await db.temp_opn_tmp.update(data, {
    where: {
...
exports.dumpTempOpnTmpDataByDocRunning = async (doc_running_no) =>
  await db.sequelize.query(
    `insert into temp_opn_tmp
